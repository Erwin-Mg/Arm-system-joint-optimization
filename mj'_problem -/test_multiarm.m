...
% X = rand(1,24)*length_robot/3;
tic;
point_coordinate = X(7:end);
t = 1;
W =N;
...
f=[];
l=length_robot/2;  % 夹持物体的长度
% robotArm=robotArm_initial;
% endlocation=endlocation_1;
% N=10000;
%
% 设立 关节刚度K_q
K_q=2.8e4*ones(1,joint_number);
% 创建四个单臂机器人
...
line_A = point_coordinate(13:15);
line_B = point_coordinate(16:18);
line_C = point_coordinate(19:21);
line_D = point_coordinate(22:24);
% 单位化连线向量
vector_A = line_A / norm(line_A);
vector_B = line_B / norm(line_B);
vector_C = line_C / norm(line_C);
vector_D = line_D / norm(line_D);

n_A=cross(vector_A,[0,0,1])';
o_A=cross(n_A,vector_A)';
R_A=tr2eul([n_A,o_A,vector_A']);
trans_matixA = rotz(R_A(1)*180/pi)*roty(R_A(2)*180/pi)*rotz(R_A(3)*180/pi);


n_BA=cross(vector_B,[0,0,1])';
o_BA=cross(n_BA,vector_B)';
R_BA=tr2eul([n_BA,o_BA,vector_B']);
trans_matixBA = rotz(R_BA(1)*180/pi)*roty(R_BA(2)*180/pi)*rotz(R_BA(3)*180/pi);

n_CA=cross(vector_C,[0,0,1])';
o_CA=cross(n_CA,vector_C)';
R_CA=tr2eul([n_CA,o_CA,vector_C']);
trans_matixCA = rotz(R_CA(1)*180/pi)*roty(R_CA(2)*180/pi)*rotz(R_CA(3)*180/pi);

n_DA=cross(vector_D,[0,0,1])';
o_DA=cross(n_DA,vector_D)';
R_DA=tr2eul([n_DA,o_DA,vector_D']);
trans_matixDA = rotz(R_DA(1)*180/pi)*roty(R_DA(2)*180/pi)*rotz(R_DA(3)*180/pi);

% 为每个机器人臂定义基坐标系相对于正四面体底面顶点的变换矩阵
T_arm1 = transl(vertex_A) * trotz(R_A(1)*180/pi)*troty(R_A(2)*180/pi)*trotz(R_A(3)*180/pi);
...
endlocation_1(:,4:end)=endlocation(:,4:end);
% 获得剩余三臂工作空间
Q_2=zeros(W,j+3);
Q_3=zeros(W,j+3);
Q_4=zeros(W,j+3);
...
if isempty(points_recover)
    ...
    for i=1:myLength(coop12recover)
    ...
    for i=1:myLength(coop13recover)
    ...
    for i=1:myLength(coop14recover)
    ...
    for i=1:myLength(coop23recover)
    ...
    for i=1:myLength(coop24recover)
    ...
    for i=1:myLength(coop34recover)
...
%% 
    ...
    if ~isempty(coop12)
        for i=1:myLength(coop12)
            if length(J12_2{i})==joint_number
    ...
    if ~isempty(coop14)
        for i=1:myLength(coop14)
            if length(J14_4{i})==joint_number
    ...
    if ~isempty(coop23)
        for i=1:myLength(coop23)
            if length(J23_3{i})==joint_number
    ...
    if ~isempty(coop24)
        for i=1:myLength(coop24)
            if length(J24_4{i})==joint_number
    ...
    if ~isempty(coop34)
        for i=1:myLength(coop34)
            if length(J34_4{i})==joint_number
